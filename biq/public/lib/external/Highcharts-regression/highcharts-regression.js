/**
 * This plugin was originated at https://github.com/phpepe/highcharts-regression and has been modified to support the specific data formats
 * used in our implementation of scatter plots for the dashboard and BIQ.  The areas where changes have been made are described in more detail
 * inline, but the primary change was to access the x and y values for the point using dot notation rather than an array index.  This
 * was necessary because we also include a name with each point rather than just supplying a tuple.  Note, these changes have only been
 * made for linear regression and not the other regression functions.
 * 
 * Also note, this library is licensed under the MIT license.
 * 
 * Demetrios Kotsikopoulos - Sentrana, Inc. 
 */
...
(function (H) {
    	...
    	for (i = 0 ; i < series.length ; i++){
    		...
    		if ( s.regression && !s.rendered ) {
    			...
    			var extraSerie = {
    					...
    					name: s.name + ": "+ (s.regressionSettings.label || "Equation: %eq"),
                ...
                //Disabled rSquared computation to save time - it is being computed in the linear regression function.
            	regression.rSquared =  0; //coefficientOfDetermination(s.data, regression.points).toFixed(3);
            	regression.rValue =  0; //Math.sqrt(regression.rSquared,2).toFixed(3) ;
	...
	function _linear(data) {
        ...
        for (; n < data.length; n++) {
          if (data[n].y) {
          /* Note - this is where the primary changes to the downloaded version were made by Sentrana. Before the points were referenced using data[n][0] or 
           * data[n][1].  Similar changes were made for the calculation of the r-squared as well.
           */
            sum[0] += data[n].x; //Σ(X) 
            sum[1] += data[n].y; //Σ(Y)
        ...
        for (var i = 0, len = data.length; i < len; i++) {
            var coordinate = [data[i].x, data[i].x * gradient + intercept];
            results.push(coordinate);
        }
        var string = 'Slope=' + Math.round(gradient*1000) / 1000 + ', Intercept=' + Math.round(intercept*1000) / 1000 + 
        ', R-squared=' + Math.round(rSquared*1000) / 1000;
    ...
    function coefficientOfDetermination (data, pred ) {
    	/*
    	 * Note, this section was also modified by Sentrana to support our scatter plot implementation.
    	 */
		var i = SSE = SSYY =  mean = 0;

		// Calc the mean
		for (i = 0 ; i < data.length ; i++ ){
		    mean +=  data[i].y / data.length ;
		}
