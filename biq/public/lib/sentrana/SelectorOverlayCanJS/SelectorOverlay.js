(function() {
	...
	$.Controller.extend("Sentrana.Controllers.SelectorOverlay", {
		pluginName: "sentrana_selector_overlay",
		...
		defaults: {
			...
			tmplBase: "selectorBase.ejs",			/* This is the template file that initializes a table before converting to a DataTables. */
			tmplOverlay: "selectorOverlay.ejs",	/* This is the template file that defines how a grand total row is added to the table's footer section. */
		...
		updateView: function SO_updateView() {
			...
			this.element.html(can.view(this.getTemplatePath(this.options.tmplBase), {text: this.options.selectedName}));
		...
		generateOverlay: function() {
			...
			// Invoke our template...
			$("body").append(can.view(this.getTemplatePath(this.options.tmplOverlay), {
