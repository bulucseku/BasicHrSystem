Sentrana.Controllers.FilterControlCalendar("Sentrana.Controllers.FilterControlCalendarOnlyWhen", {
    pluginName: "sentrana_filter_control_calendar_only_when",
    defaults: {}
}, {
    ...
    buildElementsUI: function () {
        ...
        this.$onlywhenfilterControl.append(can.view('templates/filterCalendarContainer.ejs', {
            dateFormat: this.dateFormatCaptionText,
            hid: form.hid
        }));
        $(".date-range-list", this.$onlywhenfilterControl).append(can.view('templates/filterCalendar.ejs', {
            index: this.rangeIndex
        }));
        // Link to add more date ranges
        this.$onlywhenfilterControl.append('<a href="#" class="calendar-add-additional-range">Add additional date range</a>');
    ...
    showHideDateFormatCaption: function () {
        ...
        if ($(".date-range-list", this.$onlywhenfilterControl).children('.filterCalendarContainer').length > 0) {

            this.$onlywhenfilterControl.children('.dateformat').css("display", "block");
        }
        else {

            this.$onlywhenfilterControl.children('.dateformat').css("display", "none");
        }
    ...
    '.hasDatepicker keypress': function (el, ev) {
        ...
        if (event.keyCode === 13) {
            ...
            $(el, this.$onlywhenfilterControl).datepicker().focus();
            ...
            $(el, this.$onlywhenfilterControl).datepicker("show");
        }
    ...
    toggleDatePickerVisibility: function (inputElement) {
        ...
        if ($(inputElement, this.$onlywhenfilterControl).datepicker("widget").is(":visible")) {

            $(inputElement, this.$onlywhenfilterControl).datepicker("hide");
        }
        else {
            $(inputElement, this.$onlywhenfilterControl).datepicker("show");
        }
    },
    initCalendarControl: function () {
        var that = this;

        $(".element-filter-calendar-from", this.$onlywhenfilterControl).datepicker({
            changeMonth: true,
            changeYear: true,
            yearRange: 'c-20:c+20',
            onSelect: function (dateText, inst) {
                ...
                $(this, this.$onlywhenfilterControl).datepicker("hide");
            },
        ...
        $(".element-filter-calendar-to", this.$onlywhenfilterControl).datepicker({
            changeMonth: true,
            changeYear: true,
            yearRange: 'c-20:c+20',
            onSelect: function (dateText, inst) {
    ...
    handleToDateChange: function (changedToDate, control) {
        ...
        $(".element-filter-calendar-from[index=" + index + "]", this.$onlywhenfilterControl).datepicker("option", "maxDate", changedToDate);
        $(".element-filter-calendar-from[index=" + index + "]", this.$onlywhenfilterControl).datepicker("setDate", fromDate);
    },
    ...
    handleFromDateChange: function (changedFromDate, control) {
        ...
        $(".element-filter-calendar-to[index=" + index + "]", this.$onlywhenfilterControl).datepicker("option", "minDate", changedFromDate);
        $(".element-filter-calendar-to[index=" + index + "]", this.$onlywhenfilterControl).datepicker("setDate", upperBound);
    },

    generateValidFromDate: function (toDate, index) {

        var fromDate = $.datepicker.formatDate(this.dateFormat, $(".element-filter-calendar-from[index=" + index + "]", this.$onlywhenfilterControl).datepicker("getDate"));
    ...
    generateValidToDate: function (fromDate, index) {
        ...
        var toDate = $.datepicker.formatDate(this.dateFormat, $(".element-filter-calendar-to[index=" + index + "]", this.$onlywhenfilterControl).datepicker("getDate"));
    ...
    // Browser Event: What to do when a user removes one of the date range...
    '.remove-range click': function (el, ev) {
        var $fromCalendarControl = $(".element-filter-calendar-from", $(el).parents().parents().parents().eq(0));
        var $toCalendarControl = $(".element-filter-calendar-to", $(el).parents().parents().eq(0));
    ...
    "{dwSelection} change": function (dwSelectionModel, ev, attr, how, newVal, oldVal) {
        ...
        if (match) {
            ...
            if (match[3] != this.options.form.oid) {
            ...
            if (how === "add") {
                ...
                if (!this.rangeList[index]) {
                    ...
                    this.initCalendarControl();
                    var $fromCalendarControl = $('.element-filter-calendar-from[index=' + index + ']', '.' + this.options.form.hid);
                    var $toCalendarControl = $('.element-filter-calendar-to[index=' + index + ']', '.' + this.options.form.hid);
                ...
                var $dateRangeControl = $('.element-filter-calendar-from[index=' + index + ']', '.' + this.options.form.hid);
                $dateRangeControl.parents().parents().eq(0).remove();
            }
        }
        this.showHideDateFormatCaption();
    }
