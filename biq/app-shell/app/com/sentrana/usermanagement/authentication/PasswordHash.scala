 ...
 */
class PasswordHash(val salt: Array[Byte], var hash: Array[Byte], var pbkdf2Iterations: Int) {
  /**
  ...
  def checkPassword(password: String): Boolean = {
    val testHash: Array[Byte] = PasswordHash.computeHash(password, salt, pbkdf2Iterations, hash.length)
  ...
  def toBase64String(): String = {
    String.format("%s:%s:%s", pbkdf2Iterations.toHexString, PasswordHash.toBase64(salt), PasswordHash.toBase64(hash))
  }
  ...
  def slowEquals(a: Array[Byte], b: Array[Byte]): Boolean = {
    ...
    for (i <- 0 until math.min(a.size, b.size) - 1)
      diff |= (a(i) ^ b(i)).toInt
    diff == 0
  }
}
...
object PasswordHash {
  ...
  def randomString(alphabet: String)(n: Int): String =
    Stream.continually((new scala.util.Random).nextInt(alphabet.size)).map(alphabet).take(n).mkString
